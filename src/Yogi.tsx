...
const Yogi: React.FC = () => {
  ...
  const monthIndex = today.getMonth();     // 0 = Jan, matches your DB logic
  const year = today.getFullYear();

  const buttons = [
    { angle: 0, route: "/dashboard", label: "Chat With AI" },
    { angle: 120, route: "/manage-finances", label: "Manage Finances" },
    { angle: 240, route: "/future-plan", label: "Investment Advice" },
  ];
        ...
        {buttons.map((btn, idx) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                if (btn.route === "/future-plan") {
                  // 🔹 Pass username, month, year as state
